...
namespace RDP.Networking.Shared.Net.ConnectionManagement {
	...
	public class ServerGameNetPortal : MonoBehaviour {
		[SerializeField] private NetworkObject m_GameState;
		...
		public void OnNetworkReady() {
			if (!m_Portal.NetManager.IsServer) {
				...
				NetworkManager.Singleton.SceneManager.LoadScene("NetworkingLobby", LoadSceneMode.Single);

				if (m_Portal.NetManager.IsHost) m_ClientSceneMap[m_Portal.NetManager.LocalClientId] = ServerScene;
			}
		}
		...
		public bool AreAllClientsInServerScene() {
			foreach (KeyValuePair<ulong, int> kvp in m_ClientSceneMap)
		...
		public string GetPlayerName(ulong clientId, int playerNum) {
			PlayerData? playerData = GetPlayerData(clientId);
		...
		private void ApprovalCheck(byte[] connectionData, ulong clientId,
			...
			if (m_ClientData.ContainsKey(connectionPayload.clientGUID)) {
				if (Debug.isDebugBuild) {
					Debug.Log(
						$"Client GUID {connectionPayload.clientGUID} already exists. Because this is a debug build, we will still accept the connection");
					while (m_ClientData.ContainsKey(connectionPayload.clientGUID))
		...
		private void ServerStartedHandler() {
			...
			NetworkObject gameState = Instantiate(m_GameState);

			gameState.Spawn();
		}

		private static void AssignPlayerName(ulong clientId, string playerName) {
