...
namespace RDP.Networking.Shared.Net.ConnectionManagement {
	...
	public class ClientGameNetPortal : MonoBehaviour {
		...
		public event Action<string>
			OnUnityRelayJoinFailed; // todo put UI code as its own assembly so code can reference it. In theory, UI should be consumed by everyone
			...
			if (!NetworkManager.Singleton.IsConnectedClient && !NetworkManager.Singleton.IsHost) {
				...
				if (SceneManager.GetActiveScene().name != "MainMenu") {
					...
					SceneManager.LoadScene("MainMenu");
				           ...
				           DisconnectReason.Reason == ConnectStatus.Undefined) {
		...
		public static bool StartClientRelayMode(GameNetPortal portal, string roomKey, out string failMessage) {
			...
			string region = splits[0];
			string roomName = splits[1];
		...
		public static void ReceiveServerToClientSetDisconnectReason_CustomMessage(
