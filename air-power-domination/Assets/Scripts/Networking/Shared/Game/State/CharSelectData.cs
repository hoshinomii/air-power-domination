...
namespace RDP.Networking.Shared.Game.State {
	...
	public class CharSelectData : NetworkBehaviour {
		...
		public struct LobbyPlayerState : INetworkSerializable, IEquatable<LobbyPlayerState> {
			...
			public LobbyPlayerState(ulong clientId, string name, int playerNum, SeatState state, int seatIdx = -1,
		...
		public void ChangeSeatServerRpc(ulong clientId, int seatIdx, bool lockedIn) {
			// foreach( var del in OnClientChangedSeat.GetInvocationList() )
			// {
			//     Debug.Log( $"Delegate - Method: {del.Method} Target {del.Target}" );
			// }
