...
using RDP.Networking.Client.UI;
using RDP.Networking.Shared.Game.State;
...
namespace RDP.Networking.Client.Game.State {
	/// <summary>
	...
	public class ClientCharSelectState : GameStateBehaviour {
		...
		public override GameState ActiveState => GameState.NetworkingLobby;
		public CharSelectData CharSelectData { get; private set; }

		[SerializeField] [Tooltip("This is triggered when the player chooses a character")]
		private string m_AnimationTriggerOnCharSelect = "BeginRevive";

		[SerializeField] [Tooltip("This is triggered when the player presses the \"Ready\" button")]
		...
		[SerializeField] [Tooltip("Text element containing player count which updates as players connect")]
		private TextMeshProUGUI m_NumPlayersText;

		[SerializeField] [Tooltip("Text element for the Ready button")]
		private TextMeshProUGUI m_ReadyButtonText;

		[Header("UI Elements for different lobby modes")]
		[SerializeField]
		[Tooltip("UI elements to turn on when the player has not chosen their seat yet. Turned off otherwise!")]
		...
		[Header("Misc")] [SerializeField] [Tooltip("The controller for the class-info box")]
		private UICharSelectClassInfoBox m_ClassInfoBox;

		[SerializeField] private Transform m_CharacterGraphicsParent;

		private int m_LastSeatSelected = -1;
		private bool m_HasLocalPlayerLockedIn = false;

		private GameObject m_CurrentCharacterGraphics;

		private Animator m_CurrentCharacterGraphicsAnimator;

		private Dictionary<Guid, GameObject> m_SpawnedCharacterGraphics = new Dictionary<Guid, GameObject>();
		...
		protected override void Start() {
			base.Start();
			for (int i = 0; i < m_PlayerSeats.Count; ++i) m_PlayerSeats[i].Initialize(i);
		...
		private void UpdatePlayerCount() {
			...
			string pstr = count > 1 ? "PLAYERS" : "PLAYER";
		...
		private void OnLobbyPlayerStateChanged(NetworkListEvent<CharSelectData.LobbyPlayerState> changeEvent) {
			...
			if (localPlayerIdx == -1) {
				...
				UpdateCharacterSelection(CharSelectData.LobbyPlayers[localPlayerIdx].SeatState,
					CharSelectData.LobbyPlayers[localPlayerIdx].SeatIdx);
			}
		}
		...
		private void UpdateCharacterSelection(CharSelectData.SeatState state, int seatIdx = -1) {
			...
			if (state == CharSelectData.SeatState.Inactive) {
				if (m_CurrentCharacterGraphics) m_CurrentCharacterGraphics.SetActive(false);
				...
				if (seatIdx != -1)
					...
					if (isNewSeat) {
						// GameObject selectedCharacterGraphics = GetCharacterGraphics(CharSelectData.AvatarConfiguration[seatIdx]);
				...
				if (state == CharSelectData.SeatState.LockedIn && !m_HasLocalPlayerLockedIn) {
					...
					ConfigureUIForLobbyMode(CharSelectData.IsLobbyClosed.Value
						? LobbyMode.LobbyEnding
						: LobbyMode.SeatChosen);
		...
		private void UpdateSeats() {
			...
			foreach (CharSelectData.LobbyPlayerState playerState in CharSelectData.LobbyPlayers) {
				...
				if (curSeats[playerState.SeatIdx].SeatState == CharSelectData.SeatState.Inactive
				    || curSeats[playerState.SeatIdx].SeatState == CharSelectData.SeatState.Active &&
		...
		private void OnLobbyClosedChanged(bool wasLobbyClosed, bool isLobbyClosed) {
			if (isLobbyClosed) ConfigureUIForLobbyMode(LobbyMode.LobbyEnding);
		}
		...
		private void ConfigureUIForLobbyMode(LobbyMode mode) {
			...
			foreach (List<GameObject> list in m_LobbyUIElementsByMode.Values) {
				foreach (GameObject uiElement in list) uiElement.SetActive(false);
			}

			foreach (GameObject uiElement in m_LobbyUIElementsByMode[mode]) uiElement.SetActive(true);
			...
			switch (mode) {
				case LobbyMode.ChooseSeat:
					if (m_LastSeatSelected == -1) {
						if (m_CurrentCharacterGraphics) m_CurrentCharacterGraphics.gameObject.SetActive(false);
						m_ClassInfoBox.ConfigureForNoSelection();
					}
			...
			foreach (UICharSelectPlayerSeat seat in m_PlayerSeats)
		...
		public void OnPlayerClickedReady() {
			...
			CharSelectData.ChangeSeatServerRpc(NetworkManager.Singleton.LocalClientId, m_LastSeatSelected,
				!m_HasLocalPlayerLockedIn);
			Debug.Log($"{m_LastSeatSelected}, {!m_HasLocalPlayerLockedIn}");
		}
		...
		private void OnValidate() {
			if (
				...
				1) // Hacky way for checking if this is a scene object or a prefab instance and not a prefab definition.
