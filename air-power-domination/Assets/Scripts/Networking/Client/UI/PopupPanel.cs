...
namespace RDP.Networking.Client.UI {
	...
	public class PopupPanel : MonoBehaviour {
		...
		[SerializeField] [Tooltip("The nested Text component of m_InputField, where we stick the placeholder text")]
		private TMP_Text m_InputFieldPlaceholderText;

		[SerializeField] [Tooltip("The Port-number input field for the panel")]
		...
		[SerializeField] [Tooltip("This Button appears for popups that ask for player inputs")]
		private Button m_CancelButton;

		[SerializeField] private NameDisplay m_NameDisplay;
		...
		private bool m_EnterAsHost;

		private string m_IpHostMainText;
		private string m_RelayMainText;
		private string m_UnityRelayMainText;
		...
		/// </summary>
		private Action<string, int, string, OnlineMode> m_ConfirmFunction;

		private const string k_DefaultConfirmText = "CONFIRM";
		...
		public void SetupEnterGameDisplay(bool enterAsHost, string titleText, string ipHostMainText,
		                                  string relayMainText, string unityRelayMainText, string inputFieldText,
		                                  string confirmationText,
		                                  Action<string, int, string, OnlineMode> confirmCallback,
		...
		private void RelayRadioRadioButtonPressed(bool value) {
			if (!value) return;
		...
		private void OnConfirmClick() {
			...
			if (portNum <= 0) portNum = m_DefaultPort;
		...
		public void SanitizeInputText() {
			string inputFieldText = Sanitize(m_InputField.text, k_InputFieldIncludeChars);
			m_InputField.text = inputFieldText;
		}
		...
		public void SanitizePortText() {
			string inputFieldText = Sanitize(m_PortInputField.text);
			...
			// }
				...
				if (task == null || task.IsFaulted || caughtException != null) {
					...
					if (Application.isEditor)
						...
						SetupNotifierDisplay(
							"UNITY RELAY ERROR!",
							"Something went wrong trying to reach Unity Relay. Please follow the instructions here https://docs-multiplayer.unity3d.com/docs/develop/relay/relay/index.html#how-do-I-enable-Relay-for-my-project" +
							"to setup Unity Relay and use relay mode.", false, true);
					else
						// If there is no photon app id set tell the user they need to install
						SetupNotifierDisplay(
							"UNITY RELAY ERROR!",
							"Something went wrong trying to reach Unity Relay. It needs to be setup in the Unity Editor for this project " +
							"by following the Unity Relay guide, then rebuild the project and distribute it.", false,
							true);
		...
		private string GenerateRandomRoomKey() {
			...
			for (int i = 0; i < 6; i++) {
				char val = Convert.ToChar(Random.Range(65, 90));
				sb.Append(val);
			}
		...
		public void SetupNotifierDisplay(string titleText, string mainText, bool displayImage, bool displayConfirmation,
		                                 string subText = "") {
			ResetState();
