using RDP.Networking.Client.Game.State;
...
namespace RDP.Networking.Client.UI {
	...
	public class UICharSelectPlayerSeat : MonoBehaviour {
		[SerializeField] private GameObject m_InactiveStateVisuals;
		[SerializeField] private GameObject m_ActiveStateVisuals;
		[SerializeField] private Image m_PlayerNumberHolder;

		[SerializeField] private TextMeshProUGUI m_PlayerNameHolder;

		//[SerializeField]
		//private Image m_Glow;
		[SerializeField] private Image m_Checkbox;
		...
		[SerializeField] private Vocation m_CharacterClass;
		...
		public void SetState(CharSelectData.SeatState state, int playerIndex, string playerName) {
			if (state == m_State && playerIndex == m_PlayerNumber) return; // no actual changes

			m_State = state;
			m_PlayerNumber = playerIndex;
			m_PlayerNameHolder.text = playerName;
			if (m_State == CharSelectData.SeatState.Inactive) m_PlayerNumber = -1;
			ConfigureStateGraphics();
		}
		...
		private void ConfigureStateGraphics() {
			...
			} else // Either active or locked-in... these states are visually very similar
			{
				...
				m_PlayerNumberHolder.sprite = ClientCharSelectState.Instance
					.m_IdentifiersForEachPlayerNumber[m_PlayerNumber].Indicator;
				m_ActiveStateVisuals.SetActive(true);

				m_PlayerNameHolder.gameObject.SetActive(true);
				m_PlayerNameHolder.color = ClientCharSelectState.Instance
					.m_IdentifiersForEachPlayerNumber[m_PlayerNumber].Color;
				m_Button.interactable = m_IsDisabled ? false : true;
