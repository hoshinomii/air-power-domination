...
namespace RDP.Networking.Server.Game.State {
	/// <summary>
	...
	public class ServerCharSelectState : GameStateBehaviour {
		...
		private void OnClientChangedSeat(ulong clientId, int newSeatIdx, bool lockedIn) {
			...
			if (newSeatIdx == -1)
				...
				foreach (CharSelectData.LobbyPlayerState playerInfo in CharSelectData.LobbyPlayers)
					if (playerInfo.ClientId != clientId && playerInfo.SeatIdx == newSeatIdx &&
					    playerInfo.SeatState == CharSelectData.SeatState.LockedIn) {
			...
			foreach (CharSelectData.LobbyPlayerState r in CharSelectData.LobbyPlayers)
				Debug.Log($"{r.ClientId}, {r.PlayerName}, {r.PlayerNum}, {r.SeatState}");
		...
		public override void OnNetworkDespawn() {
			...
			if (CharSelectData) CharSelectData.OnClientChangedSeat -= OnClientChangedSeat;
		}
		...
		private int GetAvailablePlayerNum() {
			for (int possiblePlayerNum = 0; possiblePlayerNum < CharSelectData.k_MaxLobbyPlayers; ++possiblePlayerNum) {
				...
				if (!found) return possiblePlayerNum;
			}
		...
		private void SeatNewPlayer(ulong clientId) {
			...
			if (playerNum == -1)
				...
				throw new Exception(
					$"we shouldn't be here, connection approval should have refused this connection already for client ID {clientId} and player num {playerNum}");

			string playerName = m_ServerNetPortal.GetPlayerName(clientId, playerNum);
			CharSelectData.LobbyPlayers.Add(new CharSelectData.LobbyPlayerState(clientId, playerName, playerNum,
				CharSelectData.SeatState.Inactive));
		}
